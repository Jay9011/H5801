	...
	<section class="container section scrollspy" id="intro">
		<div class="row">
			...
			<div class="col s10">
				...
				<form name="bookFrm"
					action="${pageContext.request.contextPath}/Payment/pay.ho"
					method="post">
							...
							<c:otherwise>
								...
								<c:forEach var="dto" items="${book }">
									...
									<tr>
										<td>#${dto.rnum }</td>
										<%--<td>${dto.p_uid }번</td>--%>
										<td>${dto.b_seatType }<br> (번호: ${dto.t_name })
										</td>
										<td>${dto.b_sdate }</td>
										<td>${dto.b_term }시간</td>
										<td>${dto.m_nick }</td>
										<td><fmt:formatNumber value="${dto.total_amount }"
												pattern="#,###" />원</td>
										<td><c:if test="${dto.p_cancel==0 }">
					-
					</c:if> <c:if test="${dto.p_cancel==1 }">
					결제완료
					</c:if> <c:if test="${dto.p_cancel==2 }">
					결제취소
					</c:if></td>

										<td><c:if test="${dto.b_refund==0 } ">
												<p>
													<label> <input class="with-gap" id="p_uid"
														name="p_uid" type="radio" value="${dto.p_uid }"
														disabled="disabled" /> <span></span>
													</label>
												</p>
											</c:if> <c:if test="${dto.b_refund==1 }">
												<p>
													<label> <input class="with-gap" id="p_uid"
														name="p_uid" type="radio" value="${dto.p_uid }" /> <span></span>
													</label>
												</p>
											</c:if></td>
						...
						<div class="pager center">
							<ul class="pagination">
								<c:if test="${ curPageNum > 5 }">
									<li><a
										href="${pageContext.request.contextPath}/MyPage/book.ho?page=${ blockStartNum - 1 }"
										class='tooltip-top'><i class='material-icons'>chevron_left</i></a></li>
								</c:if>
								...
								<c:forEach var="i" begin="${ blockStartNum }"
									...
									<c:choose>
										...
										<c:otherwise>
											<li><a
												href="${pageContext.request.contextPath}/MyPage/book.ho?page=${ i }">${ i }</a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								...
								<c:if test="${ lastPageNum > blockLastNum }">
									<li><a
										href="${pageContext.request.contextPath}/MyPage/book.ho?page=${ blockLastNum + 1 }"
										class='tooltip-top'><i class='material-icons'>chevron_right</i></a></li>
								</c:if>
							</ul>
						</div>
					</div>
			...
			<div class="col s12 14 center-align">
				...
				<div class="row">
					<div class="col s12 right-align">
						<button type="button" id="btn1" class="btn waves-effect"
							style="margin-right: 5px;" onclick="chkPaySubmit()">결제</button>
						<button type="button" id="btn2" class="btn waves-effect"
							onclick="chkPayCancelSubmit()">취소</button>
					</div>
				</div>
			</div>
			<div class="col s1 "></div>
	...
	<c:otherwise>
		...
		<div id="demo-modal" class="modal">
			...
			<div class="modal-footer">
				<a href="${pageContext.request.contextPath}/User/login.ho"
					class="modal-close waves-effect waves-green btn-flat amber">확인</a>
			</div>
		</div>
